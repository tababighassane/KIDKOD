...
const Vis = () => {
  ...
  useEffect(() => {
    ...
    loader.load("/src/components/static/models/Angry.fbx", function (object) {
    ...
    loader.load("/src/components/static/models/sailor.fbx", function (object) {
    ...
    loader.load("/src/components/static/models/ARROW.fbx", function (object) {
    ...
    //adding the car
    gltfLoader.load("/src/components/static/models/car.glb", function (gltf) {
      mixer2 = gltf.scene;
    ...
    function addMorph(mesh, clip, speed, duration, x, y, z, fudgeColor, type) {
      mesh = mesh.clone();
    ...
    gltfloader.load("/src/components/static/models/Stork.glb", function (gltf) {
      const mesh = gltf.scene.children[0];
      const clip = gltf.animations[0];
  ...
  function quests() {
    ...
    if (task === "four") return <Game2 close={close} />;
    if (task === "five") return <DnDBasket close={close} />;
    if (task === "six") return <CubeTowerGame close={close} />;
  }

  document.onkeydown = function (e) {
    if (e.keyCode === 13 && level === 1) {
      settask("one");
      setlevel(0);
    }
    if (e.keyCode === 13 && level === 2) {
      settask("two");

      setlevel(0);
    }
    if (e.keyCode === 13 && level === 3) {
      settask("three");

      setlevel(0);
    }
    if (e.keyCode === 13 && level === 4) {
      settask("four");
      setlevel(0);
    }
    if (e.keyCode === 13 && level === 5) {
      settask("five");
      setlevel(0);
    }
    if (e.keyCode === 13 && level === 6) {
      settask("six");
      setlevel(0);
    }
  };
  ...
  function changeImage() {
    if (image === 0) {
      return (
        <div className="image image-load">
          <div className="loadcontainer">
            <video
              loading="lazy"
              muted="muted"
              src="https://cdnl.iconscout.com/lottie/premium/thumb/loading-motion-3585810-3162487.mp4"
              width="279.8"
              height="279.8"
              type="video/mp4"
              autoplay="autoplay"
            ...
            ></video>
            <img
              src="https://cdn.discordapp.com/attachments/925110224573042718/938084283074113647/waqw.png"
              id="loadingtext"
            />
          </div>
        </div>
